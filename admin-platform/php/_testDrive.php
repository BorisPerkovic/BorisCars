...
if(isset($_GET['funkcija']))
{
    ...
    if($function=="right_part")
    {
        ...
        if($db->num_rows($res)!=0)
        {
            while($row=$db->fetch_object($res))
            {
                ...
                {
                   $output.="<p><button class='btn' id='accept_testDrive'>Prihvati vožnju</button> | <button class='btn' id='deny_testDrive'>Odbaci vožnju</button></p>"; 
                }
            }
    ...
    //FUNCTION FOR RIGHT PART DATA ACCEPTING TEST DRIVE
    if($function=="accept_testDrive")
    {
        $testDrive_id=$_POST['testDrive_number'];
        $user=$_SESSION['admin_name'];
        $sql="UPDATE test_drive SET serviced='Voženo', testDrive_admin='$user' WHERE testDrive_id='$testDrive_id'";
        $db->query($sql);
        Log::upisiLog("../../logs/servisi.txt", "$user je prihvatio test vožnju modela sa ID $testDrive_id");
    }

    //FUNCTION FOR RIGHT PART DATA DENY TEST DRIVE
    if($function=="deny_testDrive")
    {
        $testDrive_id=$_POST['testDrive_number'];
        $user=$_SESSION['admin_name'];
        $sql="UPDATE test_drive SET serviced='Odbijeno', testDrive_admin='$user' WHERE testDrive_id='$testDrive_id'";
        $db->query($sql);
        Log::upisiLog("../../logs/servisi.txt", "$user je odbacio test vožnju modela sa ID $testDrive_id");
    }
}
?>